...
use cli_core::{
	...
	rivet_api::{
		self,
		apis::cloud_devices_links_api::{
			CloudDevicesLinksCompleteError, CloudDevicesLinksGetError,
			CloudDevicesLinksPrepareError,
		},
		models,
	},
};
...
impl SubCommand {
	pub async fn execute(&self, ctx: &cli_core::Ctx, term: &Term) -> Result<()> {
		...
		match self {
			SubCommand::SignIn => {
				...
				// Prepare the link
				let prepare_res =
					rivet_api::apis::cloud_devices_links_api::cloud_devices_links_prepare(
				...
				if let Err(err) = prepare_res.as_ref() {
					struct_fmt::print(
						&Format::Json,
						&SideKickResponse::Err(SideKickResponseErr::APIError(
							format!("{err:?}"),
						)),
					)?;
				}
				...
				let token = loop {
					let prepare_res =
						rivet_api::apis::cloud_devices_links_api::cloud_devices_links_get(
							&openapi_config_cloud_unauthed,
				...
				// Inspect the token
				let inspect_res = rivet_api::apis::cloud_auth_api::cloud_auth_inspect(
