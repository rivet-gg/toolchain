...
./bin/test-cmd.sh version read-config -o "cdn.site=\"$SITE_ID\"" -o "matchmaker.docker.image=\"$BUILD_ID\""

# MARK: Create version
VERSION_NAME="$(git rev-parse --abbrev-ref HEAD) $(git rev-parse --short HEAD)"
VERSION_NAME="$(date -u +"%Y-%m-%dT%H:%M:%S")"
VERSION_ID="$(./bin/test-cmd.sh version create --display-name "$VERSION_NAME" -o "cdn.site=\"$SITE_ID\"" -o "matchmaker.docker.image=\"$BUILD_ID\"" --format json | jq -r '.version_id')"
