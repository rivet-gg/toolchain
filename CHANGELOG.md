...
- Added `sidekick get-bootstrap-data` subcommand to get the initial data about
  the signed-in user
- Added `sidekick get-cli-version` subcommand to get the version of the Rivet CLI
...
- Added `sidekick get-version` subcommand to get the manage version URL in the hub
...
- Added hidden `Sidekick` command to be used by external tools (e.g. engine
