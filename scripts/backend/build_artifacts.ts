...
export async function buildArtifacts(
	...
	Object.keys(archiveFileSizes)
		...
		.forEach((key) => console.log(`[${logName}]`, key, Math.ceil(archiveFileSizes[key]! / 1000) + "KB"));
}
...
async function buildDynamicArchive() {
	const outputPath = resolve(projectRoot(), "artifacts", "dynamic_archive.json");
...
async function buildRuntimeArchive() {
	const outputPath = resolve(projectRoot(), "artifacts", "packages_archive.json");
	await buildArtifacts({
		logName: "packages",
