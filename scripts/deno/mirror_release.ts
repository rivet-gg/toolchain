...
const GITHUB_REPO = "https://github.com/denoland/deno";
const BUCKET_NAME = "rivet-releases";
const BUCKET_FOLDER = "deno";
...
async function mirrorRelease(releaseName: string) {
  const githubReleaseName = `v${releaseName}`;
  ...
  const releaseUrl = `https://api.github.com/repos/denoland/deno/releases/tags/${githubReleaseName}`;
  ...
  assert(assets.length > 0, `No files found for release ${githubReleaseName}. Please check the release name and try again. You can find a list of all releases at ${GITHUB_REPO}/releases.`);
