...
fn main() {
    ...
    assert!(status.success(), "Building process-supervisor package failed");
    ...
    println!("cargo:rustc-env=PROCESS_SUPERVISOR_BINARY_PATH={}", binary_path.display());

    // Calculate and output SHA256 hash of the binary
    let mut file = File::open(&binary_path).expect("Failed to open binary file");
    let mut buffer = Vec::new();
    file.read_to_end(&mut buffer).expect("Failed to read binary file");
